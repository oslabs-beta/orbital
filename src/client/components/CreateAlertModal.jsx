...
function MyAlerts({ setShowMyAlerts }) {
  ...
  useEffect(() => {
    axios
      .get('/alerts/' + localStorage.getItem('userId'))
      .then((res) => {
        console.log({ alerts });
        setAlerts(res.data);
      })
    ...
    >
      <Card sx={styles.card}>
        <CardContent>
          ...
          <List>
            {!alerts.length ? (
              <Box>No Alerts Here!</Box>
            ) : (
              alerts.map((alert) => (
                <ListItem>
                  <ListItemText
                    primary={
                      alertNames[alert.metric] +
                      ' Over: ' +
                      alert.over +
                      ' Under: ' +
                      alert.under
...
export default function CreateAlertModal({ setShowModal, showModal }) {
  ...
  const createAlert = async () => {
    ...
    const res = await axios.post('/alerts', body).catch((e) => {
      console.log(e);
    });
  };

  return showMyAlerts ? (
    <MyAlerts setShowMyAlerts={setShowMyAlerts} />
  ) : (
    <Modal
      open={showModal}
        ...
        <CardContent>
          ...
          <FormControl fullWidth>
            ...
            >
              ...
              <MenuItem value={'producerRequestsTotal'}>
                Total Producer Requests
              </MenuItem>
              <MenuItem value={'producersMessagesInTotal'}>
                Total Producer Messages
              </MenuItem>
              <MenuItem value={'producerConversionsTotal'}>
                Total Producer Conversions
              </MenuItem>
              <MenuItem value={'consumerRequestsTotal'}>
                Total Consumer Requests
              </MenuItem>
              <MenuItem value={'consumerConversionsTotal'}>
                Total Consumer Conversions
              </MenuItem>
              <MenuItem value={'consumerFailedRequestsTotal'}>
                Total Failed Consumer Requests
              </MenuItem>
            </Select>
          </FormControl>
